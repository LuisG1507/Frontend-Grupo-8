...
export class ContractUpdate implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.cS.listId(this.id).subscribe((data) => {
      this.contract = data;
      this.contract.startDate = new Date(data.startDate);
      this.contract.endDate = new Date(data.endDate);
      this.contract.createdAt = data.createdAt.toString().substring(0, 16);
    });
  }
